...
local ships = {
  ...
  { chosen = 'fire', confirmOrder = 1, range = 'FAR' },
}
...
ok(#fleet.resolveOrder({ { chosen = 'fire' } }) == 1, 'resolveOrder handles a 1-ship fleet')
...
ok(fleet.broadsideReady(near('fire', 'fire'), false), 'both FIRE + both NEAR should arm BROADSIDE')
ok(not fleet.broadsideReady(near('fire', 'fire'), true), 'BROADSIDE fires at most once per battle')
ok(not fleet.broadsideReady(near('fire', 'move'), false), 'BROADSIDE needs both captains on FIRE')
local split = near('fire', 'fire')
split[2].range = 'FAR'
ok(not fleet.broadsideReady(split, false), 'BROADSIDE needs both ships NEAR')
ok(not fleet.broadsideReady({ { chosen = 'fire', range = 'NEAR' } }, false),
...
ok(fleet.autoChoice({ patched = false }) == 'fire', 'auto-choice fires when seaworthy')

-- Round gate: every ship must have chosen before resolution starts.
ok(not fleet.allChosen({ { chosen = 'fire' }, {} }), 'allChosen waits on the undecided captain')
ok(fleet.allChosen({ { chosen = 'fire' }, { chosen = 'fix' } }), 'allChosen passes once both picked')
