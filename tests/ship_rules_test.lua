...
ok(data.SHIPCLASSES.sloop.hullBase == 12, 'SLOOP hullBase is 12')
ok(data.SHIPCLASSES.brig.hullScale == 6, 'BRIG hullScale is 6')
ok(data.SHIPCLASSES.fireship.weak == 'round', 'FIRESHIP weakness is round')
ok(data.SHIPCLASSES.manowar.armor == 2, 'MAN-O-WAR armor is 2')
...
ok(eStats.maxHp == 12 + 4 * 3, 'sloop hp scales correctly: 24')
ok(eStats.guns == 1, 'sloop guns is 1')
